...
hdfs dfs -ls "$directorio_datos" | while read -r archivo; do
	...
	if [ ! -f "$directorio_datos_local/$archivo_semanal" ]; then
	    > "$directorio_datos_local/$archivo_semanal"
	fi
...
python "$archivo_mapreducer" -r hadoop "hdfs:///user/alumno/$directorio_datos/$archivo_semanal" --accion "$nombre_accion" 
